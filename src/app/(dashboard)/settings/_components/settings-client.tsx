...
import { User, Palette, Loader2, Flag, Plus, X, Bot } from "lucide-react"
import { cn } from "@/lib/utils"
import { toast } from "sonner"
import { useRouter } from "next/navigation"
import SettingsCard from "./settings-cards"
import SettingsRow from "./settings-rows"
import LlmProviderClient from "../llm-provider/_components/llm-provider-client"
...
interface SettingsClientProps {
  ...
  initialLlmConfig: LlmProviderConfig
  defaultTab?: string
  enableCvTailor?: boolean
}
...
export default function SettingsClient({
  ...
  const [activeTab, setActiveTab] = useState<"general" | "red-flags" | "llm-provider">(
    enableCvTailor && (defaultTab === "llm-provider" || defaultTab === "ai")
      ? "llm-provider"
      : defaultTab === "red-flags"
      ? "red-flags"
      : "general"
  )

  const handleTabChange = (tab: "general" | "red-flags" | "llm-provider") => {
    setActiveTab(tab)
    ...
    <div className="flex-1 overflow-y-auto w-full p-6 scroll-smooth bg-[#F8F7F5]">
      <div className="max-w-3xl mx-auto flex flex-col gap-6 animate-in fade-in duration-200">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-[#6B6863]">Manage your profile details, application preferences, and AI engine.</p>
        </div>

        {/* Navigation Tabs */}
        <div className="flex items-center gap-1.5 border-b border-[#E8E6E0] pb-2">
          <button
            type="button"
            ...
            className={cn(
              ...
              activeTab === "general"
                ? "bg-[#FFF0F0] text-[#FF6B6B] shadow-2xs font-bold"
                : "text-[#6B6863] hover:text-[#2D2D2D] hover:bg-[#F4F3F0]"
            )}
          >
          ...
          <button
            type="button"
            onClick={() => handleTabChange("red-flags")}
            className={cn(
              "px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-2 cursor-pointer",
              activeTab === "red-flags"
                ? "bg-[#FFF0F0] text-[#FF6B6B] shadow-2xs font-bold"
          ...
          >
            ...
            <span>Manage Red Flags</span>
          </button>

          {enableCvTailor && (
            <button
              type="button"
              onClick={() => handleTabChange("llm-provider")}
              className={cn(
                "px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-2 cursor-pointer",
                activeTab === "llm-provider"
                  ? "bg-[#FFF0F0] text-[#FF6B6B] shadow-2xs font-bold"
                  : "text-[#6B6863] hover:text-[#2D2D2D] hover:bg-[#F4F3F0]"
              )}
            >
        ...
        {/* TAB 1: GENERAL PROFILE & APP PREFERENCES */}
        {activeTab === "general" && (
        ...
        {/* TAB 3: AI & LLM PROVIDER ENGINE */}
        {enableCvTailor && activeTab === "llm-provider" && (
          <div className="flex flex-col gap-6 animate-in fade-in duration-150">
            <LlmProviderClient initialConfig={initialLlmConfig} isEmbedded={true} />
          </div>
        )}
